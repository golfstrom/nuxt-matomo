...
export default <%= options.blockLoading ? 'async ' : ''%>(context, inject) => {
  ...
  const tracker = createTracker(context)
  if (!tracker) return
  <% } else { %>
  let tracker
  if (window.Piwik) {
    ...
    Object.defineProperty(window, 'Piwik', {
      ...
      set (newVal) {
        ...
        _windowPiwik = newVal
        _tracker = createTracker(context, delayedCalls)
        delayedCalls = void 0
      }
    })
  }
...
function createTracker (context, delayedCalls = []) {
  if (!window.Piwik) {
    <% if(debug || options.debug) { %>
  ...
  if(typeof trackerUrl === 'function') {
    trackerUrl = trackerUrl(context);
  }

  if(typeof siteId === 'function') {
    siteId = siteId(context);
  }

  const tracker = window.Piwik.getTracker(trackerUrl, siteId)
  ...
  <% if(debug || options.debug) { %>
  debug(`Created tracker for siteId ${siteId} to ${ trackerUrl }`)
  <% if(options.verbose) { %>
